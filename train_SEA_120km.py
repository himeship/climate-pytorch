...
spatial_disc = Spatial_Discriminator(in_channels=2, conditioning_steps=cond_steps)
temporal_disc = Temporal_Discriminator(in_channels=2, conditioning_steps=cond_steps)
disc = Discriminator(spatial_disc, temporal_disc).to(device, dtype)
print('Disc: {}'.format(disc.dtype), flush=True)

Load_path = 'SEA_120km_model_state_dict_1'
Save_path = 'SEA_120km_model_state_dict_2'
...
def custom_collate_fn(batch):
    ...
    return {
        'wind': torch.stack([s['wind'] for s in valid_samples]),
        'prcp': torch.stack([s['prcp'] for s in valid_samples]),
        'refl': torch.stack([s['refl'] for s in valid_samples]),
    }, valid_dates
...
for epoch in range(NUM_EPOCHS):
    ...
    for batch_idx, (samples, dates) in enumerate(loader):

        # Optional: make per-GPU noise different
        torch.manual_seed(local_rank + epoch * 1000 + batch_idx)
         
        wind = samples['wind'].to(device)
        prcp = samples['prcp'][:, :(cond_steps+fore_steps)].to(device)
        refl = samples['refl'][:, :(cond_steps+fore_steps)].to(device)
        wind = wind * (wind<=999)
        prcp = prcp * (prcp<=999)
        refl = refl * (refl>=-30)
        refl = torch.cat([refl, refl.max(dim=2, keepdim=True).values], dim=2)
        video = torch.cat([prcp, refl], dim=2)
        
        context_wind, context, real = wind[:, :cond_steps].to(dtype), video[:, :cond_steps].to(dtype), prcp[:, cond_steps:].to(dtype)
        cur_batch_size = video.shape[0]
        
        # time regime
        dates_tensor = torch.Tensor([list(map(int, [t[4:6], t[6:8], t[8:10], t[10:12], t[12:14]])) for t in dates])
        dates_tensor = dates_tensor.to(device).to(dtype)
        ...
        context_real = prcp.repeat_interleave(TRAJECTORY, dim=0)
        ...
        for _ in range(DISC_ITER):
            with autocast(dtype=dtype):  # Use BFloat16 for A100
                ...
                real_output = disc(context_real_detached, dates_tensor)
                fake_output = disc(context_fake_detached, dates_tensor)
        ...
        with autocast(dtype=dtype):  # Use BFloat16 for A100; Float32 for H100
            regularizer1 = torch.mean(50*torch.log(torch.abs(fake - real)+1)*torch.clip(torch.log(real+1), 0, 5)) + torch.mean(torch.abs(fake - real)*torch.clip(real+1, 0, 128))
            regularizer2 = torch.mean(50*torch.log(torch.abs(fake - real)+1)*torch.clip(torch.log(fake+1), 0, 5)) + torch.mean(torch.abs(fake - real)*torch.clip(fake+1, 0, 128))
            loss_gen = amp * (2*torch.mean(1-disc(context_fake, dates_tensor.to(dtype))) + regularizer1*LAMBDA1 + regularizer2*LAMBDA2)
        ...
        if is_main_process:
            ...
            if ((batch_idx+1) % 1 == 0) & (batch_idx >= 0):
                ...
                loss_d_cpu = loss_disc.cpu().float().detach().numpy().reshape(1)
                loss_g_cpu = loss_gen.cpu().float().detach().numpy().reshape(1)
                loss_d_list.append(loss_d_cpu)
                loss_g_list.append(loss_g_cpu)
                LOSS_D = np.concatenate(loss_d_list)
                LOSS_G = np.concatenate(loss_g_list)
